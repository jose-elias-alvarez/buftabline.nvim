...
local iterate_maps = function(max, key, command)
    for i = 0, max do
        map("n", replace_idx(key, i), replace_idx(command, i))
    end
end

local set_maps = function()
    if o.get().go_to_maps then
        iterate_maps(9, "<Leader><idx>", ":lua require'buftabline'.go_to_buffer(<idx>)<CR>")
    end
    if o.get().kill_maps then
        iterate_maps(9, "<Leader>c<idx>", ":lua require'buftabline'.kill_buffer(<idx>)<CR>")
    end
    if o.get().custom_command and o.get().custom_map_prefix then
        iterate_maps(
            9,
            "<Leader>" .. o.get().custom_map_prefix .. "<idx>",
