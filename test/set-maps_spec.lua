...
describe("set_maps", function()
    ...
    it("should set go to buffer maps when go_to_maps == true", function()
        ...
        for i = 0, 9 do
            assert.equals(vim.fn.maparg("<Leader>" .. i), ":lua require'buftabline'.go_to_buffer(" .. i .. ")<CR>")
        end
    end)
    ...
    it("should set kill buffer maps when kill_maps == true", function()
        ...
        for i = 0, 9 do
            assert.equals(vim.fn.maparg("<Leader>c" .. i), ":lua require'buftabline'.kill_buffer(" .. i .. ")<CR>")
        end
    end)

    it("should set custom command maps when custom_command and custom_map_prefix have been set", function()
        o.set({ custom_command = "vsplit", custom_map_prefix = "v" })

        set_maps()

        for i = 0, 9 do
            assert.equals(vim.fn.maparg("<Leader>v" .. i), ":lua require'buftabline'.custom_command(" .. i .. ")<CR>")
        end
    end)
end)
